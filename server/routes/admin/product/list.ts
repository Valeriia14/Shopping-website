...
export default controller(async (req: Request, res: Response) => {
  const { categories, inventory, search } = req.query!;
  ...
  const products = await model_utils.scope(Product, "admin").findAll({
    ...options,
  });

  let _products: any[] = [];
  const promises = products.map(async (product: Product) => {
    const product_detail = await SvProduct.modify_product_response({ product, is_admin: true });
