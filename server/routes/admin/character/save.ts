...
export default controller([
  ...
  const updated = await transact().run(async (transaction) => {
    ...
    // update category
    const { name, description, meta_keywords, meta_title, type, published, gender, asset_id } = category_body;
    ...
    const updated_character = await SvClassification.update_character({
      character, name, description, handle : path, meta_keywords,
      meta_title, type, extras, actor_id: self.id, published,
      published_at! : published? timestamp : null, asset_id, gender
