...
import { format_path, slugify, validator } from "@kidztime/utilities";
...
export default controller([
  ...
  const updated = await transact().run(async (transaction) => {
    ...
    // update category
    const { name, description, meta_keywords, meta_title, type, published, asset_id } = category_body;
    ...
    const updated_category = await SvClassification.update_category({
      category, name, description, handle : path, meta_keywords,
      meta_title, type, extras, actor_id: self.id, published,
      published_at! : published? timestamp : null, asset_id
