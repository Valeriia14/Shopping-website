...
export default (props: any) => {
  ...
  const feature_banner = useMemo(() => {
    const assets = category?.webpage?.items[0]?.assets ?? [];
    let elements = category?.webpage?.items[0]?.options?.feature_banner;
        ...
        <Hidden>
          <Box mt={2}>
            <NavBreadcrumbs
              firstTitle="Home"
              firstLink="/"
              finalTitle={category?.name ?? pascalize(win?.location?.pathname.slice(10))}
            />
          </Box>
        </Hidden>
        {category ? (
          ...
          <NotFoundPage title="Oops!Can not found this category." />
        )}
      </Container>
      <ContactBox />
    </Box>
